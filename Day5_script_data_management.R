...
# create a unique data set (both of the following do the same-thing; clean data)
cleandata <- distinct(agriculture1)
cleandata2 <- unique(agriculture1)

# appending data in R (serves to combine different data in one)
agric_AB <- bind_rows(agriculture_townA,agriculture_townB)
# Exercise: check for duplicates in combined data
# to check for duplicates in the data we run the following
duplicated(agric_AB)
...
# Exercise:1.gender and location, crop_type and location, gender and crop_Type
...
chisq.test(table(agric_gender$Crop_Type, agric_gender$gender))

# t-test for equality of means (t-test is used when the data is normally distributed)
...
# Exercise:2.using agric_gender data, t.test on gender vs yield_tonnes
shapiro.test(agric_gender$yield_tonnes)
t.test(yield_tonnes ~ gender, data = agric_gender)

# Wilcoxon test for equality of medians (for non-parametric test)
